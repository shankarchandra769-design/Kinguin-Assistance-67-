...
async def setpanelimage(ctx):
    ...
    # Upload the image to a private channel or use the CDN URL directly
    # Discord CDN URLs from attachments expire, so we re-upload to a temp message to get a stable URL
    import io
    file_bytes = await attachment.read()
    file = discord.File(io.BytesIO(file_bytes), filename=attachment.filename)

    # Send image to current channel as a hidden reference message, then save its CDN url
    temp_msg = await ctx.send(file=discord.File(io.BytesIO(file_bytes), filename=attachment.filename))
    stable_url = temp_msg.attachments[0].url

    cfg = get_config()
    cfg["ticket_panel_image_url"] = stable_url
    save_config(cfg)

    # Delete the temp message so it's clean
